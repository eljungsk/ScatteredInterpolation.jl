...
- The per-cell candidate patch lists (`cellpatches`) from Task 2 were removed: they are
  dense in higher dimensions (the patch radius spans ~2 cells per dimension, so each
  patch registers in most cells), making construction O(cells × patches). Covering
  patches are found exactly with `inrange` against the KDTree of patch centers that
  the interpolant stores anyway. Tasks 4 and 5 below are already updated accordingly.
- `buildgrid` calibrates the per-side cell count by the ball/cell volume ratio so mean
  patch occupancy tracks `pointsperpatch` in any dimension.
- Per-patch solves run with BLAS pinned to one thread (`withpinnedblas`); local
  interpolants are stored with a concrete element type.
...
widening the evaluation cutoff), (3) the `inrange` candidate query (its radius must be
`grid.radius`, the same one used at assignment).
